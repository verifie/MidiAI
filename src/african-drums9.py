...
class MelodicMixPlayer(RhythmicMixPlayer):
    ...
    def _generate_melodic_phrase(self, rhythm_name, start_note_idx):
        ...
        rhythmic_pattern = random.choice(MELODY_RHYTHMS[rhythm_name])
        phrase = []; current_note_idx = start_note_idx
        for i, step in enumerate(motif):
            if i >= len(rhythmic_pattern): break
            current_note_idx += step
            current_note_idx = max(0, min(len(MELODY_SCALE) - 1, current_note_idx))
            note = MELODY_SCALE[current_note_idx]
            beat = rhythmic_pattern[i]
