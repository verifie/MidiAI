...
# Beethoven-inspired melodic patterns (9th and 7th Symphony)
BEETHOVEN_SYMPHONY = [
    (72, [(0,1),(4,1),(7,2),(12,1),(7,1),(4,2)], VEL_LOUD),        # Joyful theme (9th)
    (67, [(0,2),(2,1),(3,1),(5,2),(3,1),(2,1)], VEL_MEDIUM),     # Reflective theme (7th)
    (64, [(7,1),(5,1),(4,2),(2,1),(0,2)], VEL_SOFT),             # Thoughtful theme (7th)
    (48, [(5,1),(7,1),(9,1),(7,1),(5,1),(4,1),(2,2)], VEL_MEDIUM) # Deep bass theme (9th)
]
...
# Generate a structured symphony

def generate_symphony():
    melody_events = []
    current_time = START_TIME
    for base_note, motif, velocity in BEETHOVEN_SYMPHONY:
        for degree, duration in motif:
            note = base_note + degree
            melody_events.append((current_time, note, duration * SECONDS_PER_BEAT, velocity))
...
def main():
    ...
    symphony_events = generate_symphony()

    voices = [
        VoicePlayer('soprano', midiout, symphony_events),
        VoicePlayer('alto', midiout, symphony_events),
        VoicePlayer('tenor', midiout, symphony_events),
        VoicePlayer('bass', midiout, symphony_events),
        VoicePlayer('accomp', midiout, symphony_events)
    ]

    for v in voices:
        v.start()

    print("Playing Beethoven-inspired piano symphony based on the 9th and 7th Symphonies. Enjoy the performance. Ctrl+C to stop.")
    try:
        time.sleep(300)  # Play for approximately 5 minutes
    except KeyboardInterrupt:
        print("Stopping performance...")
