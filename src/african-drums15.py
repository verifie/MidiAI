...
# AI Prompt: in option 9 please omit rhythm 2
...
MELODIC_PHRASES = {
    'Fanga (Liberia/Guinea)': {
        'Theme A': {'calls': [[(0.5, 2), (1.5, 4), (2.5, 3)]], 'responses': [[(0.5, 1), (1.5, 0), (2.5, -1)]], 'harmonies': [[(0.5, 0), (2.5, 1)]]},
        'Theme B': {'calls': [[(0.75, 4), (2.25, 2), (3.25, 1)]], 'responses': [[(0.75, 2), (1.75, 1), (3.25, 0)]], 'harmonies': [[(0.75, 2), (2.25, 0)]]}
    },
    'Kpanlogo (Ga - Ghana)': {
        'Theme A': {'calls': [[(0, 0), (0.75, 2), (1.5, 4)]], 'responses': [[(0.75, 3), (1.75, 2), (2.75, 1), (3.75, 0)]], 'harmonies': [[(0, -2), (1.5, 0)]]},
        'Theme B': {'calls': [[(0.5, 4), (1.5, 3), (2.5, 2)]], 'responses': [[(0.5, 1), (1.5, 0), (3.0, 2), (3.5, 1)]], 'harmonies': [[(0.5, 2), (2.5, 1)]]}
    },
    'Agbekor (Ewe - Ghana)': {
        'Theme A': {'calls': [[(0, 3), (3, 5), (6, 4)]], 'responses': [[(0, 4), (3, 3), (6, 2), (9, 1)]], 'harmonies': [[(0, 1), (6, 2)]]},
        'Theme B': {'calls': [[(2, 2), (5, 1), (8, 0)]], 'responses': [[(2, 0), (5, -1), (8, -2), (11, 0)]], 'harmonies': [[(3, -1), (9, 0)]]}
    },
    'Bikutsi (Cameroon)': {
        'Theme A': {'calls': [[(0, 4), (1.5, 5), (3, 4)]], 'responses': [[(0, 2), (1.5, 1), (3, 0)]], 'harmonies': [[(0, 0), (3, 2)]]},
        'Theme B': {'calls': [[(0.5, 2), (2, 3), (4, 1)]], 'responses': [[(0.5, 3), (2, 2), (3.5, 1), (5, 0)]], 'harmonies': [[(1.5, -1), (4.5, 0)]]}
    },
    'Shiko (Nigeria)': {
        'Theme A': {'calls': [[(0, 0), (1.5, 2), (2.5, 3)]], 'responses': [[(0, 1), (1, 0), (2.5, 2)]], 'harmonies': [[(0, -2), (2.5, 0)]]},
        'Theme B': {'calls': [[(0.75, 4), (2, 2), (3, 1)]], 'responses': [[(0.75, 2), (1.5, 1), (3.25, 0)]], 'harmonies': [[(0.75, 1), (3.25, -1)]]}
    },
    'Wassoulou (Mali)': {
        'Theme A': {'calls': [[(0, 2), (1, 3), (2, 4)]], 'responses': [[(0, 1), (1, 0), (2, -1)]], 'harmonies': [[(0, 0), (2, 2)]]},
        'Theme B': {'calls': [[(0.5, 5), (2.5, 3)]], 'responses': [[(0.5, 3), (1.5, 2), (2.5, 1), (3.5, 0)]], 'harmonies': [[(0.5, 3), (2.5, 1)]]}
    },
    'Moribayassa (Guinea)': {
        'Theme A': {'calls': [[(0.75, 5), (1.75, 6), (2.75, 5)]], 'responses': [[(0.75, 4), (1.75, 3), (2.75, 2), (3.75, 1)]], 'harmonies': [[(0.75, 3), (2.75, 3)]]},
        'Theme B': {'calls': [[(0.5, 2), (1.5, 3), (2.5, 4)]], 'responses': [[(0.5, 1), (1.5, 0)]], 'harmonies': [[(0.5, 0), (2.5, 2)]]}
    }
}
...
class RhythmicMixPlayer(threading.Thread):
    ...
    def _handle_deconstruct(self, current_rhythm, parts_with_channel):
        ...
        # Deconstruct faster
        for _ in range(len(build_order) - 1): 
             if self.stop_event.is_set(): return
             build_filter = lambda p_name: p_name in active_parts
             # Play only a short section
             self._play_section(current_rhythm, 1, part_filter=build_filter, custom_parts=parts_with_channel.items())
             if len(active_parts) > 1: active_parts.pop(random.randint(1, len(active_parts)-1)) # remove a random non-core part
    ...
    def _handle_buildup(self, current_rhythm):
        ...
        # Build up the next rhythm over just 1 or 2 measures
        build_up_measures = 2
        for i in range(build_up_measures):
            if self.stop_event.is_set(): return
...
class MelodicMixPlayer(RhythmicMixPlayer):
    def __init__(self, scheduler, rhythm_list):
        ...
        self.is_call = True
        self.groove_measure_counter = 0
        self.THEME_CHANGE_THRESHOLD = 6 # Change theme faster
        self.theme_keys = {r_name: list(themes.keys()) for r_name, themes in MELODIC_PHRASES.items()}
        self.current_theme_idx = 0

    def _generate_polyphonic_phrases(self, rhythm_name, base_note_idx, lead_ch, harm_ch_1, harm_ch_2):
        current_theme_key = self.theme_keys[rhythm_name][self.current_theme_idx]
        phrase_lib = MELODIC_PHRASES[rhythm_name][current_theme_key]
        ...
        harmony_pattern_2 = [(0, -4)]
        harmony_phrase_2 = []
        for beat, degree in harmony_pattern_2:
            note_idx = max(0, min(len(MELODY_SCALE) -1, base_note_idx + degree))
            note = MELODY_SCALE[note_idx]
            harmony_phrase_2.append((beat, note, VEL_GHOST - 20, 2.0, harm_ch_2))

        return lead_phrase, harmony_phrase_1, harmony_phrase_2, base_note_idx
    
    def _handle_buildup(self, current_rhythm):
        self.channels.append(self.channels.pop(0))
        self.current_theme_idx = 0 # Reset theme for new rhythm
        self.groove_measure_counter = 0
        print(f"... Swapping melody roles. Lead is now on channel {self.channels[0] + 1} ...")
        super()._handle_buildup(current_rhythm)

    def _handle_groove(self, current_rhythm, parts_with_channel):
        play_measures = random.randint(3, 4) * 2
        for _ in range(play_measures):
            if self.stop_event.is_set(): break
            
            self.groove_measure_counter += 1
            if self.groove_measure_counter > self.THEME_CHANGE_THRESHOLD:
                num_themes = len(self.theme_keys[current_rhythm['name']])
                self.current_theme_idx = (self.current_theme_idx + 1) % num_themes
                new_theme_key = self.theme_keys[current_rhythm['name']][self.current_theme_idx]
                print(f"... Evolving melody to {new_theme_key} ...")
                self.groove_measure_counter = 0
...
def main():
    ...
    if len(available_ports) == 1:
        print(f"Defaulting to only available MIDI port: {available_ports[0]}")
        ...
        while selected_port_index == -1:
            try:
                choice = input("Enter the number of your MIDI device: ")
                port_num = int(choice)
                if 0 <= port_num < len(available_ports):
                    selected_port_index = port_num
                else:
    ...
    # --- Open Selected Port ---
    try:
        midiout.open_port(selected_port_index)
        print(f"\nMIDI Port '{available_ports[selected_port_index]}' opened.")
    except (rtmidi.InvalidPortError, rtmidi.NoPortsAvailableError) as e:
        ...
        while True:
            ...
            if key:
                ...
                if key in AFRICAN_RHYTHMS:
                    ...
                    print("Starting Melodic Mix... (Press 'r' to stop)"); all_rhythms = [AFRICAN_RHYTHMS[str(i)] for i in [1] + list(range(3, 8))]; active_player = MelodicMixPlayer(scheduler, all_rhythms); active_player.start()
            time.sleep(0.01)
