...
AFRICAN_RHYTHMS = {
    "1": {
        ...
        "bpm": 130, "beats_per_measure": 4,
        "parts": {
            "Dundun": [(0, 45, VEL_ACCENT, 0.5), (2, 45, VEL_ACCENT, 0.5)],
            "Sangban": [(1, 47, VEL_NORMAL, 0.25), (3, 47, VEL_NORMAL, 0.25)],
    ...
    "2": {
        ...
        "bpm": 110, "beats_per_measure": 12, # This is in 12/8 time
        "parts": {
    ...
    "3": {
        ...
        "bpm": 125, "beats_per_measure": 4,
        "parts": {
            "Bell": [(0, 56, VEL_ACCENT, 0.2), (1, 56, VEL_NORMAL, 0.2), (1.5, 56, VEL_NORMAL, 0.2), (2.5, 56, VEL_ACCENT, 0.2), (3, 56, VEL_NORMAL, 0.2)],
            "Conga 1": [(0, 62, VEL_ACCENT, 0.2), (2, 62, VEL_ACCENT, 0.2), (3.5, 62, VEL_ACCENT, 0.2)],
            "Conga 2": [(1, 65, VEL_NORMAL, 0.2), (1.5, 62, VEL_GHOST, 0.2), (3, 65, VEL_NORMAL, 0.2)]
        }
    },
    "4": {
        "name": "Bikutsi (Cameroon)",
        "bpm": 140, "beats_per_measure": 6, # 6/8 time
        "parts": {
            "Kick": [(0, 36, VEL_ACCENT, 0.2), (3, 36, VEL_ACCENT, 0.2)],
            "Snare": [(1.5, 38, VEL_NORMAL, 0.2), (4.5, 38, VEL_NORMAL, 0.2)],
    ...
    "5": {
        "name": "Shiko (Nigeria)",
        "bpm": 115, "beats_per_measure": 4,
        "parts": {
            "Bell": [(0, 56, VEL_ACCENT, 0.2), (0.75, 56, VEL_NORMAL, 0.2), (1.5, 56, VEL_NORMAL, 0.2), (2.5, 56, VEL_ACCENT, 0.2), (3.25, 56, VEL_NORMAL, 0.2)],
            "Low Tom": [(0, 45, VEL_ACCENT, 0.2), (2, 45, VEL_GHOST, 0.2)],
            "High Tom": [(1, 50, VEL_NORMAL, 0.2), (3, 50, VEL_NORMAL, 0.2), (3.5, 50, VEL_ACCENT, 0.2)],
        }
    },
    "6": {
        "name": "Wassoulou (Mali)",
        "bpm": 128, "beats_per_measure": 4,
        "parts": {
    ...
    "7": {
        "name": "Moribayassa (Guinea)",
        "bpm": 150, "beats_per_measure": 4,
        "parts": {
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if self.state == self.STATE_GROOVE:
                ...
                # Decide next state
                if random.random() < 0.7: # 70% chance to go to breakdown
                ...
                self.state = self.STATE_BUILDUP # Always go to buildup after breakdown

            elif self.state == self.STATE_BUILDUP:
                print("... Buildup ...")
                all_parts = list(current_rhythm['parts'].keys())
                random.shuffle(all_parts) # Add parts in random order for variety
                # Start with the core part
                ...
                self.state = self.STATE_FILL_SWAP # Always swap after a full buildup

            elif self.state == self.STATE_FILL_SWAP:
                print("... Fill and Swap ...")
                fill_pattern = random.choice(DRUM_FILLS)
                # Play a 4/4 fill at the current rhythm's tempo
                ...
                self.state = self.STATE_GROOVE # Back to grooving
...
def display_menu():
    ...
    print("-" * 60)
    mix_name = f"Dynamic Mix ({AFRICAN_RHYTHMS['1']['name']} & {AFRICAN_RHYTHMS['3']['name']})"
    print(f"  [0] {mix_name}")
        ...
        while True:
            ...
            if key:
                ...
                if key in AFRICAN_RHYTHMS:
                    ...
                    active_player = DynamicMixPlayer(scheduler, r1, r3)
                    active_player.start()
